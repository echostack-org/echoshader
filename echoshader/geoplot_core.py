...
class GridDataVisualizer:
    ...
    def utm_string_generator(self, longitude: float, latitude: float) -> str:
        ...
        logger.info(
            f"Generated UTM EPSG code: {epsg} for coordinates ({longitude}, {latitude})"
        )
        return epsg

    def create_boundary_gdf(
        self, bounds: Dict[str, List[float]], projection: str = "epsg:4326"
        ...
        boundary_gdf = gpd.GeoDataFrame(
            data={"geometry": [boundary_polygon]}, crs=projection
        )
        ...
        logger.info(
            f"Created boundary GeoDataFrame with UTM projection: EPSG:{utm_num}"
        )
    ...
    def create_grid_cells(
        ...
        logger.info(
            f"Creating grid cells within bounds: {xmin:.2f}, {ymin:.2f}, {xmax:.2f}, {ymax:.2f}"
        )
        ...
        x_indices, y_indices = np.meshgrid(
            range(1, len(x_coords) + 1), range(1, len(y_coords) + 1)
        )

        # Create GeoDataFrame
        cells_gdf = gpd.GeoDataFrame(
            {"geometry": grid_cells, "x": x_indices.ravel(), "y": y_indices.ravel()},
    ...
    def create_grid_from_bounds(
        ...
        logger.info(
            f"Creating grid with resolution: {grid_resolution['x_distance']}m x {grid_resolution['y_distance']}m"
        )

        # Create boundary
        boundary_gdf, boundary_gdf_utm, utm_num = self.create_boundary_gdf(
            bounds, projection
        )
    ...
    def aggregate_data_by_grid(
        ...
        """
        logger.info(
            f"Starting data aggregation by grid cells for dimensions: {dimensions}"
        )
        ...
        if mean_col in grouped_data.columns:
            ...
            logger.info(
                f"{first_dim} value range: {grouped_data[mean_col].min():.2f} to "
                f"{grouped_data[mean_col].max():.2f}"
            )
            logger.info(
                f"Non-zero {first_dim} cells: {sum(grouped_data[mean_col] > 0)}"
            )

        return grouped_data

    def merge_data_with_grid(
        self,
        cells_gdf: gpd.GeoDataFrame,
        ...
        # Merge data
        cells_with_data = cells_gdf.merge(aggregated_data, on=merge_keys, how="left")
        ...
        if custom_transforms:
            for new_col, transform_func in custom_transforms.items():
                try:
                    ...
                    logger.warning(
                        f"Failed to apply transformation {new_col}: {str(e)}"
                    )
    ...
    def create_grid_polygons(
        ...
        grid_polygons = gv.Polygons(
            cells_data, vdims=hover_columns, crs=ccrs.GOOGLE_MERCATOR
        ).opts(
            color=color_column,
            cmap=colormap,
            colorbar=True,
    ...
    def create_base_layers(
        ...
        if include_coastline:
            coastline = gv.Feature(cfeature.COASTLINE, crs=ccrs.PlateCarree()).opts(
                line_color=coastline_color, line_width=coastline_width
            )
    ...
    def create_visualization(
        ...
        **kwargs : dict
        ...
        """
        logger.info(
            f"Creating complete grid visualization for dimensions: {dimensions}"
        )
        ...
        if primary_dimension is None:
            ...
            if primary_dimension not in cells_with_data.columns:
                ...
                data_cols = [
                    col
                    for col in cells_with_data.columns
        ...
        cells_prepared = self.prepare_visualization_data(
            cells_with_data, primary_dimension, use_log_scale=use_log_scale
        )
        ...
        final_plot = plot.opts(title=title, width=width, height=height)

        logger.info("Visualization created successfully")
        return final_plot

    def create_visualization_from_bounds(
        self,
        df: pd.DataFrame,
        bounds: Dict[str, List[float]],
        dimensions: Union[str, List[str]],
        ...
        **kwargs : dict
        ...
        data_with_grid = gpd.sjoin(
            data_gdf_utm, cells_gdf, how="inner", predicate="within"
        )

        # Rename grid indices to match expected column names
        data_with_grid = data_with_grid.rename(
            columns={"x_right": "grid_x", "y_right": "grid_y"}
        )

        # Drop the geometry column to convert back to regular DataFrame
        df_with_grid = pd.DataFrame(
            data_with_grid.drop(columns=["geometry", "index_right"])
        )
